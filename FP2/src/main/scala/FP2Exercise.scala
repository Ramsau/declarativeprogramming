  ...
  def fibTail(a : Int) : Int = {
    def fibtailinner(n2: Int, n1: Int, n: Int): Int = n match {
      case 0 => n2
      case _ => fibtailinner(n1, n2 + n1, n - 1)
    }
    fibtailinner(0, 1, a)
  }
  ...
  def rangeTail(from : Int, to : Int) : List[Int] = {
    def range(acc: List[Int], from: Int, to: Int): List[Int] =
      if (from == to)
        acc
      else
        range((to - 1)::acc, from, to - 1)
    range(List[Int](), from, to)
  }
  ...
  def flatten(l : List[List[Int]]): List[Int] = l match {
    case List() => List()
    case List()::rest => flatten(rest)
    case (a::as)::rest => a::flatten(as::rest)
  }
  ...
  def flattenTail(l: List[List[Int]]): List[Int] = {
    def flatten(acc: List[Int], l: List[List[Int]]): List[Int] = l match {
      case List() => acc
      case List()::rest => flatten(acc, rest)
      case as::rest => flatten(acc ++ as, rest)
    }
    flatten(List(), l)
  }
  ...
  def unzip(pairList : List[(Int, Int)]) : (List[Int], List[Int]) = {
    def movePairs(as: List[Int], bs: List[Int], abs: List[(Int, Int)]): (List[Int], List[Int]) = {
      abs match {
        case List() => (as, bs)
        case (newa, newb)::tail => movePairs(as ++ List(newa), bs ++ List(newb), tail)
      }
    }
    movePairs(List(), List(), pairList)
  }
  ...
  def isAscending(l:List[Int]): Boolean = {
    def firstAscending(n: Int, l: List[Int]): Boolean = l match {
      case h::t if h >= n => firstAscending(h, t)
      case List() => true
      case _ => false
    }
    firstAscending(l.head, l)
  }
  ...
  def dotProduct(a_vec: List[Int], b_vec: List[Int]): Int = {
    def dotSum(sum: Int, a: List[Int], b: List[Int]): Int = (a, b) match {
      case (List(), List()) => sum
      case (List(), _) => 0
      case (_, List()) => 0
      case (ha::ta, hb::tb) => dotSum(sum + ha * hb, ta, tb)
    }
    dotSum(0, a_vec, b_vec)
  }
   ...
   * @return a & b
   */
  def bitwiseAnd(a: List[Int], b: List[Int]): List[Int] = (a, b) match {
    case (List(), List()) => List()
    case (ha::ta, hb::tb) if ha == 1 && hb == 1 => 1 :: bitwiseAnd(ta, tb)
    case (ha::ta, hb::tb) => 0::bitwiseAnd(ta, tb)
  }
   ...
   * @return l >> count
   */
  def bitwiseRightShift(l: List[Int], count: Int): List[Int] = 0
   ...
   */
  def mse(measured : List[Double], target : List[Double]) : Double = 0
   ...
   */
  def absList(list: List[Int]): List[Int] = list.map((e: Int) => if (e > 0) e else e * -1)
   ...
   * @return absolute values of the input list
   */
  def absListFold(list: List[Int]): List[Int] = list.foldRight(List[Int]())((e: Int, list: List[Int]) => if( e > 0) e::list else (e * -1)::list)
   ...
   */
  def keepPositive(list: List[Int]): List[Int] = list.filter((e: Int) => e > 0)
   ...
   * @return positive values from the input list
   */
  def keepPositiveFold(list: List[Int]): List[Int] = list.foldRight(List[Int]())((e: Int,  list: List[Int]) => if (e > 0) e::list else list)
