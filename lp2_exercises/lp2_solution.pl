...
fill_unary(0,_E,[]).
fill_unary(s(N), E, [E|Res]) :- fill_unary(N,E,Res).
...
multiply_list_unary(_M,[],[]).
multiply_list_unary(M,[H|T],Res) :-
    fill_unary(M,H,MH),
    multiply_list_unary(M,T,R),
    append(MH,R,Res).
...
removeDup([],[]).
removeDup([X],[X]).
removeDup([H,H|T], R) :- removeDup([H|T],R).
removeDup([H,X|T], [H|R]) :-
    H \= X,
    removeDup([X|T],R).
...
zip(_,[],[]).
zip([],_,[]).
zip([H1|T1], [H2|T2],[(H1,H2)|ZT]) :- zip(T1,T2, ZT).
